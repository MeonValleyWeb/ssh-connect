...
BEDROCK_PATHS=("web/wp" "wp" "app/uploads" "app/plugins")
...
# Function to detect if a site is a Bedrock site
is_bedrock_site() {
    local server_id="$1"
    local site_id="$2"
...
run_site_command() {
    ...
    ssh -i "$DEFAULT_IDENTITY" -t "$username@$ip" "cd $path && $command"
...
site_management_menu() {
    ...
    echo -e "3. Run command on all Bedrock sites"
    echo -e "4. Run composer command on all Bedrock sites"
    ...
    case $option in
        ...
        2)
            ...
            ;;
        3)
            echo -e "\n${CYAN}Enter command to run on all Bedrock sites:${NC} "
            read -r command
            
            local sites=$(jq -r ".[$server_id].sites" "$SERVERS_FILE")
            local site_count=$(echo "$sites" | jq 'length')
            local bedrock_found=false
            
            echo -e "\n${YELLOW}Detecting Bedrock sites...${NC}"
            
            for i in $(seq 0 $(($site_count - 1))); do
                local domain=$(echo "$sites" | jq -r ".[$i].domain")
                echo -n "Checking $domain... "
                
                if is_bedrock_site "$server_id" "$i"; then
                    echo -e "${GREEN}Bedrock site!${NC}"
                    bedrock_found=true
                    run_site_command "$server_id" "$i" "$command"
                else
                    echo -e "${RED}Not a Bedrock site${NC}"
            ...
            if [ "$bedrock_found" = "false" ]; then
                echo -e "\n${RED}No Bedrock sites found on this server.${NC}"
            fi
            ;;
        4)
            ...
            select composer_cmd in "${COMPOSER_COMMANDS[@]}" "custom"; do
                ...
                echo -e "\n${YELLOW}Running '$command' on Bedrock sites...${NC}"
